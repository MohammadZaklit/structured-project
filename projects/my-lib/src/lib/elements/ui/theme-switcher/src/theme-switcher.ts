...
type PresetName = keyof typeof presets; // "Aura" | "Material" | "Lara" | "Nora"
...
export class ThemeSwitcher {
  ...
  primaryColors = computed(() => {
    const presetPalette = presets[this.themeState().preset].primitive as Record<string, any>;
    const colors = [
      'emerald',
      'green',
      'lime',
    ...
    const palettes: { name: string; palette: { [key: number]: string } }[] = [];
  ...
  onPresetChange(event: any) {
    ...
    const preset = presets[event as PresetName];
